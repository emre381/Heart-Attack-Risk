...
# En iyi modeli seçmek için doğruluk oranlarını karşılaştır
best_model = None
best_accuracy = 0

# Yapay Sinir Ağları doğruluğunu kontrol et
if accuracy > best_accuracy:
    best_accuracy = accuracy
    best_model = model  # Yapay sinir ağları modeli

# Random Forest doğruluğunu kontrol et
if rf_accuracy > best_accuracy:
    best_accuracy = rf_accuracy
    best_model = rf_model  # Random Forest modeli

# Karar Ağaçları doğruluğunu kontrol et
if dt_accuracy > best_accuracy:
    best_accuracy = dt_accuracy
    best_model = dt_model  # Karar Ağaçları modeli

# Lojistik Regresyon doğruluğunu kontrol et
if lr_accuracy > best_accuracy:
    best_accuracy = lr_accuracy
    best_model = lr_model  # Lojistik Regresyon modeli

print(f'En iyi model doğruluk oranı: {best_accuracy * 100:.2f}%')

# Kullanıcıdan veri alalım ve en iyi modelle tahmin yapalım
while True:
    ...
    try:
        ...
        print(f'Tahmin sonucu: Kalp hastalığı risk oranınız: {prediction[0]}')
